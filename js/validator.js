...
var Validator = (function() {
	var validationTypes = {
		...
		'email'			: /^[0-9a-zA-Zäåö]+([0-9a-zA-Zäåö]*[-._+])*[0-9a-zA-Zäåö]+@[0-9a-zA-Zäåö]+([-.][0-9a-zA-Zäåö]+)*([0-9a-zA-Zäåö]*[.])[a-zA-Zäåö]{2,6}$/, // Email address
		...
		'cvc'			: /^\d{3,4}$/ // Credit Card CVC code
	};
...
var FieldsEnhancer = (function($) {
	function FieldsEnhancer(element) {
		this.element = element;
		this.init();
	};

	FieldsEnhancer.prototype.init = function() {
		this.restrictInput(this.element);
		this.countChars(this.element);
	};
	
	FieldsEnhancer.prototype.restrictInput = function(element) {
		if ( element.getAttribute('data-type') !== null && element.getAttribute('maxlength') !== null ) {
			var inputType = element.getAttribute('data-type');
			var inputLength = parseInt(element.getAttribute('maxlength'), 10);

			$(element).on('input', function() {
				if ( this.value.length >= inputLength ) {
					this.value = this.value.substring(0, inputLength);
				};
	...
	FieldsEnhancer.prototype.countChars = function(element) {
		if ( element.getAttribute('data-counter') !== null ) {
			$(element).on('input', function() {
				...
				$(element.getAttribute('data-counter')).text(this.value.length);
			});
		};
	};

	return FieldsEnhancer;
})(jQuery);
